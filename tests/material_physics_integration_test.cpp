...
double calculateMechanicalLoadFactor(double stress_MPa, double yield_strength_MPa,
...
double calculateCombinedDamageEffect(const struct MaterialProperties& material,
                                     double radiation_dose, double temperature_K,
                                     double mechanical_stress_MPa, bool vacuum_condition,
...
double calculateCombinedDamageEffect(const MaterialProperties& material, double radiation_dose,
                                     double temperature_K, double mechanical_stress_MPa,
                                     bool vacuum_condition, bool atomic_oxygen_present)
{
    ...
    double stressFactor = calculateMechanicalLoadFactor(mechanical_stress_MPa,
                                                        material.yield_strength, radiation_dose);
    ...
    double synergyFactor =
        calculateSynergyFactor(temperature_K, mechanical_stress_MPa, material.yield_strength);
...
void setupRadiationEnvironment(const std::string& mission_profile, RadiationEnvironment& env)
{
...
void testNASAPhysicsModels(const std::map<std::string, MaterialProperties>& materials)
{
    ...
    double combined_normal =
        calculateCombinedDamageEffect(aluminum, 1.0e4, 300.0, 50.0, false, false);
                                         ...
                                         const RadiationEnvironment& env)
{
    ...
    for (const auto& [name, material] : materials) {
        ...
        Eigen::Tensor<double, 3> source =
            setupRadiationSource(env, spatial_points, angular_points, energy_bins);
        ...
        Eigen::Tensor<double, 4> sigma_s =
            generateScatteringCrossSections(material, angular_points, energy_bins);

        // Solve transport equation
        TransportSolution solution = solveTransportEquation(fluence, source, sigma_t, sigma_s);

        // Calculate dose distribution
        Eigen::Tensor<double, 1> dose =
            calculateDoseDistribution(solution.fluence, material.density);

        // Calculate average attenuation
        double proton_attenuation = calculateAverageAttenuation(solution.fluence, ProtonParticle);
        double electron_attenuation =
            calculateAverageAttenuation(solution.fluence, ElectronParticle);
...
void testQuantumDefectFormationWithMaterials(
    const std::map<std::string, MaterialProperties>& materials, const RadiationEnvironment& env)
{
    ...
    for (const auto& [name, material] : materials) {
        ...
        CrystalLattice crystal =
            createAppropriateLatticetype(material.z_effective, lattice_constant);

        // Apply temperature-dependent displacement energy
        double temperature_adjusted_energy =
            material.calculateThresholdForTemperature(mission_temperature);
        ...
        DefectDistribution defects =
            simulateDisplacementCascade(crystal, pka_energy, params, temperature_adjusted_energy);
    ...
    const std::map<std::string, MaterialProperties>& materials, const RadiationEnvironment& env)
{
    ...
    for (const auto& [name, material] : materials) {
        ...
        double stress_factor =
            calculateMechanicalLoadFactor(applied_stress, material.yield_strength, radiation_dose);
        double synergy_factor =
            calculateSynergyFactor(temperature, applied_stress, material.yield_strength);

        std::cout << "    Temperature-adjusted threshold: " << mat_params.displacement_energy
                  << " eV" << std::endl;
        ...
        std::cout << "    Base generation rate: " << base_generation_rate << " defects/cm³/s"
                  << std::endl;
        std::cout << "    Modified generation rate: " << modified_generation_rate
                  << " defects/cm³/s" << std::endl;
        ...
        SimulationResults results =
            solveStochasticDE(concentrations, driftTerm, diffusionTerm, time_steps, simulation_time,
                              temperature, applied_stress);

        // Calculate steady-state concentration with modified generation rate
        double theoretical_ss_concentration =
            modified_generation_rate /
            (4.0 * M_PI * mat_params.diffusion_coefficient * mat_params.recombination_radius);

        std::cout << "    Final concentration: " << results.final_concentration.sum()
                  << " defects/cm³" << std::endl;
        std::cout << "    Theoretical steady-state: " << theoretical_ss_concentration
                  << " defects/cm³" << std::endl;
                                  ...
                                  const RadiationEnvironment& env)
{
    ...
    for (const auto& [name, material] : materials) {
        ...
        TimeEvolutionResults results =
            solveFieldEquations(fields, derivatives, radiation_dose, temperature, applied_stress);

        // Verify conservation laws
        double defect_difference =
            std::abs(results.total_defects_final - results.total_defects_initial);
...
void testMicromechanicalModelWithMaterials(
    const std::map<std::string, MaterialProperties>& materials, const RadiationEnvironment& env)
{
    ...
    for (const auto& [name, material] : materials) {
        ...
        CrystalLattice crystal = createAppropriateLatticetype(
            material.z_effective, calculateLatticeConstant(material.density, material.z_effective));
        ...
        double stress_factor =
            calculateMechanicalLoadFactor(calculateStressFromStrain(strain_applied, material),
                                          material.yield_strength, radiation_dose);
        ...
        Field3D<Eigen::Matrix3d> radiation_strain = calculateRadiationInducedStrain(
            vacancy_concentration, interstitial_concentration, defect_strain_tensors);
    ...
    for (const auto& mission : mission_profiles) {
        ...
        for (const auto& [name, material] : materials) {
            ...
            double damage_factor = calculateCombinedDamageEffect(
                material, radiation_dose, mission_params.temperature, stress_level,
                mission_params.is_vacuum, mission_params.has_atomic_oxygen);
            ...
            std::cout << "    Material: " << name << ", Damage: " << damage_factor
