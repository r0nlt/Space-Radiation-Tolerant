...
struct BenchmarkResults {
    ...
    std::map<std::string, PerformanceMetrics>
        performance_metrics;  ///< Performance metrics for each component
    std::map<std::string, ReliabilityMetrics>
        reliability_metrics;  ///< Reliability metrics for each component
    std::map<std::string, std::vector<double>>
        error_rates_over_time;  ///< Error rates over time for plotting
    std::map<std::string, std::vector<double>>
        correction_rates_over_time;  ///< Correction rates over time for plotting
    std::map<std::string, std::map<std::string, double>>
        comparative_metrics;  ///< For comparing different configurations
    ...
    bool exportToCSV(const std::string& filename) const
    {
        ...
        file << "Component,Execution Time (ms),Memory Usage (bytes),Energy "
                "Consumption,Throughput,Circuit Resources\n";
        for (const auto& [component, metrics] : performance_metrics) {
            file << component << "," << metrics.execution_time_ms << ","
                 << metrics.memory_usage_bytes << "," << metrics.energy_consumption << ","
                 << metrics.throughput << "," << metrics.circuit_resources << "\n";
        }
        file << "\n";

        // Write reliability metrics
        file << "Reliability Metrics\n";
        file << "Component,Total Errors,Detected Errors,Corrected Errors,Detection Rate,Correction "
                "Rate,SDC Rate,MTTF,Recovery Time\n";
        for (const auto& [component, metrics] : reliability_metrics) {
            file << component << "," << metrics.total_errors << "," << metrics.detected_errors
                 << "," << metrics.corrected_errors << "," << metrics.error_detection_rate << ","
                 << metrics.error_correction_rate << "," << metrics.silent_data_corruption_rate
                 << "," << metrics.mean_time_to_failure << "," << metrics.recovery_time << "\n";
        }
        file << "\n";
    ...
    std::string getSummary() const
    {
        ...
        if (!comparative_metrics.empty()) {
                ...
                for (const auto& [metric, value] : metrics) {
                    if (!best_configs.count(metric) ||
                        (metric.find("error") != std::string::npos &&
                         value < best_configs[metric].second) ||
                        (metric.find("error") == std::string::npos &&
                         value > best_configs[metric].second)) {
            ...
            std::vector<std::string> key_metrics = {"error_correction_rate", "execution_time_ms",
                                                    "memory_usage_bytes"};

            for (const auto& metric : key_metrics) {
                if (best_configs.count(metric)) {
                    const auto& [best_config, value] = best_configs[metric];
                    ss << "Best " << metric << ": " << best_config << " (" << std::fixed
...
class BenchmarkFramework {
   public:
     ...
     */
    BenchmarkResults runBenchmarkSuite(const std::string& test_name,
                                       ...
                                       const std::map<std::string, ProtectionConfig>& configs)
    {
        ...
        sim::EnvironmentParams rad_params =
            sim::PhysicsRadiationSimulator::getMissionEnvironment(test_scenario.mission_profile);

        // Set radiation intensity
        rad_params.solar_activity = test_scenario.radiation_intensity;

        sim::PhysicsRadiationSimulator radiation_sim(rad_params);
        ...
        for (const auto& [config_name, protection_config] : configs) {
            ...
            for (size_t i = 0; i < config_test_data.size(); ++i) {
                if (protection_config.use_selective_hardening) {
                    ...
                    protected_data.push_back(
                        std::any(rad_ml::tmr::EnhancedTMR<uint8_t>(config_test_data[i])));
                }
                else if (protection_config.use_health_weighted_tmr) {
                    protected_data.push_back(
                        std::any(rad_ml::tmr::HealthWeightedTMR<uint8_t>(config_test_data[i])));
                }
                else if (protection_config.use_approximate_tmr) {
                    protected_data.push_back(std::any(rad_ml::tmr::ApproximateTMR<uint8_t>(
                        config_test_data[i], {rad_ml::tmr::ApproximationType::EXACT,
            ...
            // Run the benchmark
            auto [perf_metrics, reliability_metrics, error_rates, correction_rates] = runBenchmark(
                config_name, test_scenario, radiation_sim, protected_data, config_test_data);
            ...
            results.comparative_metrics[config_name]["error_detection_rate"] =
                reliability_metrics.error_detection_rate;
            results.comparative_metrics[config_name]["error_correction_rate"] =
                reliability_metrics.error_correction_rate;
            results.comparative_metrics[config_name]["silent_data_corruption_rate"] =
                reliability_metrics.silent_data_corruption_rate;
            results.comparative_metrics[config_name]["execution_time_ms"] =
                perf_metrics.execution_time_ms;
            results.comparative_metrics[config_name]["memory_usage_bytes"] =
                perf_metrics.memory_usage_bytes;
            results.comparative_metrics[config_name]["energy_consumption"] =
                perf_metrics.energy_consumption;
            results.comparative_metrics[config_name]["throughput"] = perf_metrics.throughput;
            results.comparative_metrics[config_name]["circuit_resources"] =
                static_cast<double>(perf_metrics.circuit_resources);
        }
     ...
     * @return Tuple of performance metrics, reliability metrics, error rates over time, and
     * correction rates over time
     */
    std::tuple<PerformanceMetrics, ReliabilityMetrics, std::vector<double>, std::vector<double>>
    runBenchmark(const std::string& config_name, const TestScenario& test_scenario,
                 sim::PhysicsRadiationSimulator& radiation_sim,
                 std::vector<std::any>& protected_data, const std::vector<uint8_t>& original_data)
    {
        // Initialize metrics
        ...
        for (size_t iteration = 0; iteration < test_scenario.num_iterations; ++iteration) {
                ...
                for (size_t i = 0; i < protected_data.size(); ++i) {
                    ...
                    if (current_data[i] != original_data[i]) {
                        ...
                        if (protected_data[i].type() == typeid(rad_ml::tmr::EnhancedTMR<uint8_t>)) {
                            auto& tmr = std::any_cast<rad_ml::tmr::EnhancedTMR<uint8_t>&>(
                                protected_data[i]);
                            ...
                            if (current_value != original_data[i]) {
                                ...
                                // Attempt repair
                                tmr.set(original_data[i]);  // Simple repair by setting to original
                                                            // value
                        ...
                        else if (protected_data[i].type() ==
                                 typeid(rad_ml::tmr::HealthWeightedTMR<uint8_t>)) {
                            auto& tmr = std::any_cast<rad_ml::tmr::HealthWeightedTMR<uint8_t>&>(
                                protected_data[i]);
                            // Use simple check
                            uint8_t current_value = tmr.get();
                            ...
                            if (current_value != original_data[i]) {
                                ...
                                if (tmr.get() == original_data[i]) {
                                 ...
                                 typeid(rad_ml::tmr::ApproximateTMR<uint8_t>)) {
                            auto& tmr = std::any_cast<rad_ml::tmr::ApproximateTMR<uint8_t>&>(
                                protected_data[i]);
                            // Use simple check
                            uint8_t current_value = tmr.get();
            ...
            auto iter_duration =
                std::chrono::duration_cast<std::chrono::milliseconds>(end_iter - start_iter)
                    .count();
        ...
        size_t checksum_overhead_bytes =
            protected_data.size() * sizeof(uint32_t);  // 4 bytes per checksum
        perf_metrics.memory_usage_bytes =
            original_data.size() + tmr_overhead_bytes + checksum_overhead_bytes;
        ...
        if (reliability_metrics.total_errors > 0) {
            reliability_metrics.error_detection_rate =
                static_cast<double>(reliability_metrics.detected_errors) /
                reliability_metrics.total_errors;

            reliability_metrics.error_correction_rate =
                static_cast<double>(reliability_metrics.corrected_errors) /
                reliability_metrics.total_errors;
        }
        else {
        ...
        reliability_metrics.silent_data_corruption_rate =
            reliability_metrics.total_errors > 0 ? 1.0 - reliability_metrics.error_detection_rate
                                                 : 0.0;
        ...
        reliability_metrics.recovery_time =
            reliability_metrics.detected_errors > 0
                ? perf_metrics.execution_time_ms / reliability_metrics.detected_errors
                : 0.0;
        ...
        for (size_t i = 0; i < num_intervals; ++i) {
            double error_rate =
                interval_errors[i] > 0
                    ? static_cast<double>(interval_detections[i]) / interval_errors[i]
                    : 1.0;

            double correction_rate =
                interval_detections[i] > 0
                    ? static_cast<double>(interval_corrections[i]) / interval_detections[i]
                    : 1.0;
