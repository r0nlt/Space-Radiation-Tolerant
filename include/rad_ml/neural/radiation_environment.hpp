...
namespace rad_ml {
namespace neural {
// Define PI constant if not provided by <cmath>
#ifndef M_PI
...
struct RadiationFlux {
     ...
     */
    RadiationFlux()
        : electron_flux(0.0), proton_flux(0.0), heavy_ion_flux(0.0), total_dose_rate(0.0)
    {
    }
    ...
    RadiationFlux(double e_flux, double p_flux, double hi_flux, double dose)
        : electron_flux(e_flux), proton_flux(p_flux), heavy_ion_flux(hi_flux), total_dose_rate(dose)
    {
    }
     ...
     */
    double getTotalFlux() const { return electron_flux + proton_flux + heavy_ion_flux; }
    ...
    {
        return RadiationFlux(electron_flux + other.electron_flux, proton_flux + other.proton_flux,
...
struct OrbitalPosition {
    ...
    double distanceTo(const OrbitalPosition& other) const
    {
        ...
        const double a = std::sin(delta_lat / 2) * std::sin(delta_lat / 2) +
                         std::cos(lat1_rad) * std::cos(lat2_rad) * std::sin(delta_lon / 2) *
                             std::sin(delta_lon / 2);
...
class RadiationEnvironment {
   public:
    ...
    std::string getMissionName() const
    {
        switch (mission_) {
            case SpaceMission::LEO_EQUATORIAL:
                return "Low Earth Orbit (Equatorial)";
            case SpaceMission::LEO_POLAR:
                return "Low Earth Orbit (Polar)";
            case SpaceMission::MEO:
                return "Medium Earth Orbit";
            case SpaceMission::GEO:
     ...
     */
    void setSolarActivity(double activity) { solar_activity_ = std::clamp(activity, 0.0, 1.0); }
    ...
    /**
     * @brief Set average device/environment temperature in Kelvin
     * @param temperature_k Temperature in Kelvin
     */
    void setAverageTemperatureK(double temperature_k)
    {
        env_temperature_k_ = std::clamp(temperature_k, 1.0, 1000.0);
    }

    /**
     * @brief Set a device sensitivity multiplier (dimensionless)
     * @param sensitivity Typically in [0.1, 10]
     */
    void setDeviceSensitivity(double sensitivity)
    {
        device_sensitivity_ = std::max(0.0, sensitivity);
    }

    /**
     * @brief Enable or disable precise physics aggregation for SEU probability
     *
     * When enabled and a custom calculator is provided, SEU probability is
     * computed via the user-supplied functor (e.g., σ(LET)·flux with
     * corrections). Fallback remains the heuristic aggregator.
     */
    void setPrecisePhysics(bool enable) { use_precise_physics_ = enable; }

    /**
     * @brief Type of a custom SEU probability calculator
     *
     * The functor should return SEU probability per bit per second.
     */
    using SEUCalculator = std::function<double(
        const RadiationFlux& /*flux_per_cm2_per_s*/, double /*temperature_K*/,
        double /*device_sensitivity*/, bool /*in_saa_region*/, double /*solar_activity*/)>;

    /**
     * @brief Inject a precise SEU probability calculator
     */
    void setSEUCalculator(SEUCalculator calculator)
    {
        precise_seu_calculator_ = std::move(calculator);
    }
    ...
    {
        // Get radiation flux at position (particles/cm^2/s)
        RadiationFlux flux = calculateRadiationFlux(position);

        // Determine if in SAA region (non-zero SAA model output)
        const bool in_saa = (createSAAModel()(position).getTotalFlux() > 0.0);

        // If precise physics is enabled and a calculator is provided, use it
        if (use_precise_physics_ && precise_seu_calculator_) {
            // Expect per-bit per-second from calculator; convert to per-day
            const double per_sec = precise_seu_calculator_(
                flux, env_temperature_k_, device_sensitivity_, in_saa, solar_activity_);
            return per_sec * 86400.0;
        }

        // Heuristic species-weighted aggregator (per-second)
        // Coefficients loosely derived from testing; suitable for quick sims
        const double electron_factor = 1e-12;
        const double proton_factor = 1e-9;
        const double heavy_ion_factor = 1e-6;

        double seu_rate_per_sec = flux.electron_flux * electron_factor +
                                  flux.proton_flux * proton_factor +
                                  flux.heavy_ion_flux * heavy_ion_factor;

        // Apply simple corrections consistent with documented model
        const double Ctemp = 1.0 + std::max(0.0, (env_temperature_k_ - 273.0) / 100.0);
        const double Cregion = in_saa ? 1.5 : 1.0;
        const double Cdevice = std::max(0.0, device_sensitivity_);

        const double adjusted_per_sec = seu_rate_per_sec * Ctemp * Cregion * Cdevice;

        // Return per-bit per-day probability
        return adjusted_per_sec * 86400.0;
    }
    ...
    double simulateSolarFlare(double intensity = 1.0)
    {
        ...
        solar_flare_decay_ = [this, previous_activity](double time_elapsed) {
            ...
            solar_activity_ =
                previous_activity + (solar_activity_ - previous_activity) * decay_factor;
     ...
     */
    OrbitalPosition generateOrbitPosition(double inclination, double altitude,
   ...
   private:
    ...
    bool use_precise_physics_ = false;
    SEUCalculator precise_seu_calculator_{};
    ...
    void applyShielding(RadiationFlux& flux) const
    {
        ...
        flux.total_dose_rate =
            flux.electron_flux * 1e-3 + flux.proton_flux * 1e-2 + flux.heavy_ion_flux * 1e-1;
    }
    ...
    void applySolarActivityEffects(RadiationFlux& flux) const
    {
        ...
        flux.electron_flux =
            flux.electron_flux * gcr_factor + flux.electron_flux * solar_factor * 0.5;
    ...
    RadiationModelFunction createSAAModel() const
    {
        return [](const OrbitalPosition& position) -> RadiationFlux {
            ...
            return RadiationFlux(1e4 * intensity,  // Electrons
    ...
    RadiationModelFunction createGCRModel() const
    {
        return [](const OrbitalPosition& position) -> RadiationFlux {
            ...
            return RadiationFlux(base_electron * factor, base_proton * factor,
                                 base_heavy_ion * factor, base_dose * factor);
        };
    }
    ...
    RadiationModelFunction createSPEModel() const
    {
        ...
        return [](const OrbitalPosition& position) -> RadiationFlux {
            ...
            return RadiationFlux(1e6,  // Electrons
                                 1e5,  // Protons
                                 1e3,  // Heavy ions
