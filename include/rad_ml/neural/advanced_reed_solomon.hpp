...
class AdvancedReedSolomon {
   ...
   public:
    ...
    std::optional<T> decode(const std::vector<uint8_t>& encoded_data) const
    {
        ...
        for (size_t i = 0; i < total_symbols; ++i) {
            ...
            element_t symbol = 0;
            for (size_t j = 0; j < symbol_bytes && (i * symbol_bytes + j) < encoded_data.size();
                 ++j) {
        ...
        std::vector<element_t> data_symbols_vec(corrected->begin(),
     ...
     */
    constexpr size_t correction_capability() const { return ECCSymbols / 2; }
     ...
     * @return Corrupted data
     */
    std::vector<uint8_t> apply_burst_errors(const std::vector<uint8_t>& data, double error_rate,
                                            uint8_t burst_size = 3, uint64_t seed = 0) const
    {
   ...
   private:
    ...
    {
        // Proper systematic Reed-Solomon encoding
        // Compute remainder of (message * x^ECCSymbols) / generator_poly
        ...
        for (size_t i = 0; i < std::min(data_symbols, message.size()); ++i) {
            ...
            if (feedback != 0) {
                // Shift remainder and apply generator polynomial
                for (size_t j = 0; j < ECCSymbols - 1; ++j) {
                    remainder[j] = field_.add(remainder[j + 1],
...
template <typename T>
using RS4Bit8Sym =
    AdvancedReedSolomon<T, 4, 8>;  // 4-bit symbols, 8 ECC symbols (can correct 4 symbols)

template <typename T>
using RS8Bit8Sym =
    AdvancedReedSolomon<T, 8, 8>;  // 8-bit symbols, 8 ECC symbols (can correct 4 symbols)

template <typename T>
using RS8Bit16Sym =
    AdvancedReedSolomon<T, 8, 16>;  // 8-bit symbols, 16 ECC symbols (can correct 8 symbols)

template <typename T>
using RS8Bit32Sym =
    AdvancedReedSolomon<T, 8, 32>;  // 8-bit symbols, 32 ECC symbols (can correct 16 symbols)
