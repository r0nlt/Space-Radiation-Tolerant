...
enum class ErrorSeverity {
    ...
    FATAL      ///< Fatal error level (most severe)
};
    ...
    std::string getCategoryString() const
    {
        switch (category) {
            ...
            case ErrorCategory::EXTERNAL:
                return "EXTERNAL";
            case ErrorCategory::TMR:
                return "TMR";
            case ErrorCategory::NEURAL_NETWORK:
...
class RadiationFrameworkException : public std::exception {
   ...
   public:
     ...
     */
    explicit RadiationFrameworkException(ErrorInfo error_info) : error_info_(std::move(error_info))
    {
    }
...
class Result {
   public:
     ...
     */
    bool isSuccess() const { return std::holds_alternative<T>(variant_); }
     ...
     */
    bool isError() const { return std::holds_alternative<ErrorInfo>(variant_); }
...
class Result<void> {
   public:
     ...
     * @return Error result with the given error information
     */
    static Result<void> error(ErrorInfo error) { return Result<void>(std::move(error)); }
...
class ConsoleErrorLogger : public IErrorLogger {
   public:
    ...
    void logError(const ErrorInfo& error) override
    {
        ...
        if (error.severity == ErrorSeverity::DEBUG || error.severity == ErrorSeverity::INFO ||
...
#define RAD_ML_ASSERT(condition, message)                                                        \
    do {                                                                                         \
        if (!(condition)) {                                                                      \
            rad_ml::error::ErrorHandler::getInstance().logErrorImpl(                             \
                rad_ml::error::ErrorInfo(rad_ml::error::ErrorCode::VALIDATION_INVALID_OPERATION, \
                                         rad_ml::error::ErrorCategory::VALIDATION,               \
                                         rad_ml::error::ErrorSeverity::FATAL, message));         \
        }                                                                                        \
    } while (0)
...
class ErrorHandler {
   public:
     ...
     * @param error Error information
     */
    static void logError(const ErrorInfo& error) { getInstance().logErrorImpl(error); }
    ...
    static void setReportingLevel(ErrorSeverity level)
    {
        ...
        getInstance().logErrorImpl(
            ErrorInfo(ErrorCode::SUCCESS, ErrorCategory::GENERAL, ErrorSeverity::INFO,
                      "Reporting level set to " + std::to_string(static_cast<int>(level)),
    ...
    static void setLogger(Args&&... args)
    {
        // For now, just log the logger change
        getInstance().logErrorImpl(ErrorInfo(ErrorCode::SUCCESS, ErrorCategory::GENERAL,
                                             ErrorSeverity::INFO, "Logger changed",
                                             SourceLocation(__FILE__, __LINE__, __func__)));
    }
    ...
    static std::string getErrorCodeString(ErrorCode code)
    {
        switch (code) {
            ...
            case ErrorCode::MEMORY_ERROR:
                return "MEMORY_ERROR";
            case ErrorCode::RADIATION_ERROR:
            ...
            case ErrorCode::MEMORY_CORRUPTION_DETECTED:
                return "MEMORY_CORRUPTION_DETECTED";
            case ErrorCode::VALIDATION_INVALID_OPERATION:
                return "VALIDATION_INVALID_OPERATION";
            default:
    ...
    ErrorHandler()
    {
        ...
        registerCallback([](const ErrorInfo& error) {
            std::cerr << "[" << getErrorSeverityString(error.severity) << "] "
                      ...
                      << getErrorCodeString(error.code) << ": " << error.message;

            if (error.location.file_name != nullptr) {
