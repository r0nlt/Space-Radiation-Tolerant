...
class AdaptiveProtection {
   public:
    ...
    void updateEnvironment(uint32_t new_bit_flips, uint32_t new_computation_errors)
    {
        ...
        auto elapsed =
            std::chrono::duration_cast<std::chrono::seconds>(now - environment_.last_assessment)
                .count();

        if (elapsed > 0) {
            // Update flux estimate
            double error_rate =
                (new_bit_flips + new_computation_errors) / static_cast<double>(elapsed);
     ...
     *
     * Returns the currently active protection level. This level is automatically
     * adjusted based on the radiation environment, but can also be manually set.
     *
     * @return Current protection level (MINIMAL, STANDARD, ENHANCED, or MAXIMUM)
     */
    ProtectionLevel getProtectionLevel() const { return current_level_; }

    /**
     * @brief Get current radiation environment assessment
     *
     * Returns the current assessment of the radiation environment, including
     * estimated particle flux, detected bit flips, and computation errors.
     * This data is used to automatically adjust protection levels.
     *
     * @return Current radiation environment assessment with flux and error statistics
     */
    const RadiationEnvironment& getEnvironment() const { return environment_; }

    /**
     * @brief Get configuration parameters for current protection level
     *
     * Returns the configuration parameters (redundancy level, scrubbing interval,
     * temporal redundancy, checkpoint recovery) for the current protection level.
     * These parameters determine the specific protection mechanisms in use.
     *
     * @return Protection configuration with redundancy and timing parameters
     */
     ...
     *
     * Increases the protection level by one step for a specified duration.
     * This is useful for critical operations that require higher reliability
     * than the current environment would normally warrant.
     *
     * @param duration_ms How long to maintain the increased protection in milliseconds
    ...
    void temporarilyIncreaseLevel(uint32_t duration_ms = 5000)
    {
        ...
        if (current_level_ != ProtectionLevel::MAXIMUM) {
            current_level_ = static_cast<ProtectionLevel>(static_cast<int>(current_level_) + 1);
     ...
     *
     * Sets the protection level to a specific value. This overrides the
     * automatic environment-based adjustment and sets a fixed protection level.
     * The level will remain at this value until the environment changes or
     * this method is called again.
     *
     * @param level The protection level to set (MINIMAL, STANDARD, ENHANCED, or MAXIMUM)
     */
     ...
     *
     * Registers a callback function that will be called whenever the protection
     * level changes. This allows other components to react to protection level
     * adjustments, such as updating their own protection mechanisms or logging.
     *
     * @param callback Function to call when protection level changes. The function
     *                should accept a ProtectionLevel parameter.
     ...
     *
     * Removes a previously registered callback function. This should be called
     * when the callback is no longer needed to prevent memory leaks and
     * unnecessary function calls.
     *
     * @param handle The handle returned from registerLevelChangeCallback
     * @return true if callback was found and removed, false if handle was invalid
     */
