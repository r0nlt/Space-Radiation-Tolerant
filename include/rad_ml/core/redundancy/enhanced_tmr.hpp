...
class EnhancedTMR {
   public:
     ...
     */
    EnhancedTMR() noexcept : values_{T{}, T{}, T{}} { recalculateChecksums(); }
    ...
    /**
     * @brief Get the correct value using optimized majority voting
     *
    ...
    T get() const
    {
        ...
        // If all values are equal, return immediately (90%+ of cases)
        if (values_[0] == values_[1] && values_[1] == values_[2]) {
            return values_[0];
        }

        // PERFORMANCE OPTIMIZATION: Do majority voting before expensive checksum validation
        T majority_result = performMajorityVoting();

        // Only validate checksums if we have disagreement AND in high-radiation mode
        static thread_local int call_count = 0;
        call_count++;

        // PERFORMANCE OPTIMIZATION: Only check checksums every Nth call or when errors detected
        if (call_count % 100 == 0 || error_stats_.detected_errors > 0) {
            ...
            // If checksums indicate corruption, use checksum-validated result
            if (valid_count < 3) {
                incrementErrorStats(valid_count > 0);

                if (valid_count >= 2) {
                    // Use majority of valid checksums
                    for (int i = 0; i < 3; ++i) {
                        for (int j = i + 1; j < 3; ++j) {
                            if (checksum_valid[i] && checksum_valid[j] &&
                                values_[i] == values_[j]) {
                                return values_[i];
                            }
                        }
     ...
     */
    ErrorStats getErrorStats() const { return error_stats_; }
...
// PERFORMANCE OPTIMIZATION: Use SIMD for 32-bit and 64-bit integers
#if defined(__AVX2__)
        if constexpr (sizeof(U) == 4) {
            return simdBitVote32(a, b, c);
        }
        else if constexpr (sizeof(U) == 8) {
            return simdBitVote64(a, b, c);
        }
#endif
        ...
        using UintType = typename std::conditional<
            ...
            typename std::conditional<
                sizeof(U) == 4, uint32_t,
                typename std::conditional<sizeof(U) == 2, uint16_t, uint8_t>::type>::type>::type;
        ...
        // PERFORMANCE OPTIMIZATION: Use bitwise operations instead of loops
        UintType result = 0;

        // Majority voting using bitwise operations: (a&b) | (a&c) | (b&c)
        result = (a_bits & b_bits) | (a_bits & c_bits) | (b_bits & c_bits);
    ...
    static FaultPattern detectFaultPattern(const U& a, const U& b, const U& c)
    {
        ...
        else if (max_diff <= 3 && areAdjacentBits(diff_ab) && areAdjacentBits(diff_ac) &&
                 areAdjacentBits(diff_bc)) {
            return FaultPattern::ADJACENT_BITS;
        }
        else if (max_diff <= 8 && areByteBoundary(diff_ab) && areByteBoundary(diff_ac) &&
                 areByteBoundary(diff_bc)) {
            return FaultPattern::BYTE_ERROR;
        }
        else if (max_diff <= 32 && areWordBoundary(diff_ab) && areWordBoundary(diff_ac) &&
                 areWordBoundary(diff_bc)) {
    ...
    /**
     * @brief Performance benchmark for measuring TMR overhead
     *
     * @param num_operations Number of operations to benchmark
     * @return Execution time in microseconds
     */
    static double benchmarkPerformance(size_t num_operations = 1000000)
    {
        ...
        for (size_t i = 0; i < num_operations; ++i) {
            volatile int result = tmr.get();  // Prevent optimization
            (void)result;
        }
        ...
        return static_cast<double>(duration.count()) / num_operations;
    }
   ...
   private:
     ...
     */
    template <typename U>
    static U selectMostConfidentResult(const U& a, const U& b, const U& c, const U& bit_result,
                                       const U& word_result, const U& burst_result)
    {
...
// PERFORMANCE OPTIMIZATION: SIMD helper functions
#if defined(__AVX2__)
    template <typename U>
    static U simdBitVote32(const U& a, const U& b, const U& c)
    {
        static_assert(sizeof(U) == 4, "This function is for 32-bit types only");
        ...
        // Majority voting: (a&b) | (a&c) | (b&c)
        uint32_t result = (a_bits & b_bits) | (a_bits & c_bits) | (b_bits & c_bits);
    ...
    template <typename U>
    static U simdBitVote64(const U& a, const U& b, const U& c)
    {
        static_assert(sizeof(U) == 8, "This function is for 64-bit types only");
        ...
        // Majority voting: (a&b) | (a&c) | (b&c)
        uint64_t result = (a_bits & b_bits) | (a_bits & c_bits) | (b_bits & c_bits);
