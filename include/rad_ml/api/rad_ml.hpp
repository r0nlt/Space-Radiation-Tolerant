...
struct Version {
     ...
     */
    static constexpr const char* asString() { return "2.0.0"; }
};
 ...
 */
template <typename T>
inline std::shared_ptr<tmr::ApproximateTMR<T>> approximate(const T& initial_value = T(),
 ...
 */
template <typename T>
inline T* allocate(size_t count = 1, memory::MemoryProtectionLevel protection_level =
                                         memory::MemoryProtectionLevel::NONE)
{
    if (count == 1) {
        return memory::UnifiedMemoryManager::getInstance().allocateObject<T>(
            memory::MemoryFlags::DEFAULT, protection_level, "rad_ml::memory_management::allocate");
    }
    else {
        return memory::UnifiedMemoryManager::getInstance().allocateArray<T>(
            count, memory::MemoryFlags::DEFAULT, protection_level,
 ...
 */
template <typename T, typename... Args>
inline memory::RadiationTolerantPtr<T> makeProtected(memory::MemoryProtectionLevel protection_level,
                                                     Args&&... args)
{
    return memory::makeRadTolerantProtected<T>(protection_level, std::forward<Args>(args)...);
}
...
{
    error::ErrorHandler::setLogger(std::make_shared<Logger>(std::forward<Args>(args)...));
}
 ...
 */
inline void logError(error::ErrorCode code, error::ErrorCategory category,
 ...
 */
template <typename T>
inline error::Result<T> makeError(error::ErrorCode code, error::ErrorCategory category,
                                  error::ErrorSeverity severity, std::string message,
