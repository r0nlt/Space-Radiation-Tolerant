...
class EnhancedTMR {
   public:
     ...
     *
     * @param other EnhancedTMR object to copy
     */
    EnhancedTMR(const EnhancedTMR& other)
        : on_error_callback_(other.on_error_callback_),
          last_verification_time_(other.last_verification_time_),
          verification_interval_(other.verification_interval_),
          use_health_weighted_voting_(other.use_health_weighted_voting_)
    {
        ...
        for (size_t i = 0; i < num_copies_; i++) {
            ...
            crcs_[i] = other.crcs_[i];  // Changed from crc_values_ to crcs_
            health_scores_[i] = other.health_scores_[i];
        }

        // Copy statistics
        total_set_operations_ = other.total_set_operations_;
        total_get_operations_ = other.total_get_operations_;
        crc_validation_failures_ = other.crc_validation_failures_;
        voting_disagreements_ = other.voting_disagreements_;
    }
     ...
     */
    EnhancedTMR& operator=(const EnhancedTMR& other)
    {
        if (this != &other) {
    ...
    std::string getErrorStats() const
    {
        ...
        stats +=
            "  Total operations: " + std::to_string(total_set_operations_ + total_get_operations_) +
            "\n";
    ...
    T performWeightedVoting() const
    {
        ...
        if (use_health_weighted_voting_) {
            ...
            if (copy_0_1_agree || copy_0_2_agree || copy_1_2_agree) {
                ...
                double r =
                    std::uniform_real_distribution<double>(0.0, total_health)(random_engine_);
