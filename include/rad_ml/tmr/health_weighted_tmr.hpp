...
class HealthWeightedTMR {
   public:
    ...
    T get() const
    {
        // PERFORMANCE OPTIMIZATION: Fast path - if all values are equal and healthy
        if (copies_[0] == copies_[1] && copies_[1] == copies_[2] && health_scores_[0] > 0.8 &&
            health_scores_[1] > 0.8 && health_scores_[2] > 0.8) {
            return copies_[0];
        }

        // Check if checksums are still valid (only if not recently checked)
        static thread_local int checksum_counter = 0;
        checksum_counter++;

        bool use_checksums = (checksum_counter % 50 == 0);  // Check every 50th call

        if (use_checksums) {
            bool checksum_valid[num_copies_];
            for (size_t i = 0; i < num_copies_; ++i) {
                checksum_valid[i] = (checksums_[i] == calculateChecksum(copies_[i]));
                if (!checksum_valid[i]) {
                    // Decrease health score for invalid checksum
                    health_scores_[i] *= 0.9;
                    error_counters_[i]++;
                }
            }
        }

        // PERFORMANCE OPTIMIZATION: Use simple majority if health scores are similar
        double max_health = *std::max_element(health_scores_.begin(), health_scores_.end());
        double min_health = *std::min_element(health_scores_.begin(), health_scores_.end());

        if (max_health - min_health < 0.2) {  // Health scores are similar
            // Use fast majority voting
            if (copies_[0] == copies_[1]) return copies_[0];
            if (copies_[0] == copies_[2]) return copies_[0];
            if (copies_[1] == copies_[2]) return copies_[1];
        }

        // Health-weighted voting only when necessary
        return performHealthWeightedVoting();
    }
     ...
     */
    std::array<double, num_copies_> getHealthScores() const { return health_scores_; }
     ...
     */
    std::array<size_t, num_copies_> getErrorCounters() const { return error_counters_; }
   ...
   private:
     ...
     */
    T performHealthWeightedVoting() const
    {
        ...
        for (size_t i = 0; i < num_copies_; ++i) {
            weighted_votes[copies_[i]] = health_scores_[i];
