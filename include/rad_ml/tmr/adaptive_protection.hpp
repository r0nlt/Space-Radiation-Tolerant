...
class StuckBitTMR<std::vector<U>> : public TMRStrategy<std::vector<U>> {
   protected:
    ...
    std::vector<U> attemptBitCorrection(const std::vector<U>& values1,
...
class HealthWeightedTMR : public TMRStrategy<T> {
   ...
   public:
    TMRResult<T> execute(const std::function<T()>& operation) override;
    ProtectionLevel getProtectionLevel() const override
    {
...
class TMRStrategyFactory {
   public:
     ...
     */
    template <typename T>
    static std::unique_ptr<TMRStrategy<T>> createStrategy(const sim::RadiationEnvironment& env,
     ...
     */
    static ProtectionLevel calculateOptimalProtectionLevel(const sim::RadiationEnvironment& env,
                                                           const core::MaterialProperties& material,
                                                           double criticality, double temperature_K,
...
class CheckpointManager {
   ...
   public:
     ...
     */
    void adjustCheckpointInterval(const sim::RadiationEnvironment& env, double temp_factor,
                                  double stress_factor, double synergy_factor);
...
class LayerProtectionPolicy {
   ...
   public:
     ...
     */
    void optimizeProtection(const sim::RadiationEnvironment& env,
                            const core::MaterialProperties& material);
};
...
class BitProtectionStrategy {
   public:
     ...
     */
    template <typename T>
    static std::vector<int> detectStuckBits(const std::vector<T>& data,
                                            const std::vector<T>* reference = nullptr);
};
...
/**
 * @brief Compute a checksum for a value
 *
 * This function computes a simple but effective checksum for any type.
 * For arithmetic types, it uses the value directly. For complex types,
 * it computes a hash based on the byte representation.
 *
 ...
 */
template <typename T>
size_t compute_checksum(const T& value)
{
    if constexpr (std::is_arithmetic_v<T>) {
        // For arithmetic types, use the value directly
        return static_cast<size_t>(value);
    }
    else {
        // For complex types, compute a hash of the byte representation
        const uint8_t* bytes = reinterpret_cast<const uint8_t*>(&value);
        size_t checksum = 0;

        for (size_t i = 0; i < sizeof(T); ++i) {
            checksum = (checksum << 5) + checksum + bytes[i];  // Simple hash function
        }
